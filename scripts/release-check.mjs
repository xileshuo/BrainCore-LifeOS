...
if (!main.includes("单图分享唯一排版契约")) failures.push("main.js 缺少单图分享排版规则");
if (!main.includes("backdrop-filter:none !important") || !main.includes("background:#111 !important") || !main.includes("background:#fff !important")) {
  failures.push("main.js 缺少单图分享纯黑/纯白水印契约");
}
...
if (process.argv.includes("--dist")) {
  for (const profile of ["个人版", "公版（商）", "公版（免激活）", "48小时体验版"]) {
    ...
    for (const file of ["main.js", "manifest.json", "README.md", "安装说明.txt", "THIRD-PARTY-NOTICES.md"]) if (!fs.existsSync(path.join(dir, file))) failures.push(`缺少 ${profile}/${file}`);
