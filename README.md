# BrainCore LifeOS
BrainCore LifeOS is a sidebar console for time awareness, fast capture, workflows, habits, Moments notes, and knowledge review.
知识库侧边栏控制台（LifeOS）——捕捉、待办、周工作、习惯、统计与知识内化。
当前公开版本：**4.0.8 · 48 小时试用（到期需激活）**
...
In-plugin help: activation panel / Settings → About → Usage guide.

## Usage guide (detail) / 使用说明（详细）
...
配置页面用于调整插件的授权、路径、Moments、天气、模块顺序、配色和打卡等内容。桌面端标题格式为 **BrainCore 配置 · 个人版 v4.0.8**（版本号在标题末尾）。
