...
const LIFEOS_RELEASE = "2026.08.12";
...
const PLUGIN_CHANGELOG = {
    "3.2.3": [
        ...
        "文档：使用说明补充 iOS 快捷指令；仓库增加《BrainCore快捷指令使用指南》",
    ],
    "3.2.2": [
...
function renderLifeOsAboutPanel(panel, plugin, options = {}) {
  ...
  const shortcutsBlock = wrap.createDiv({ cls: "lifeos-settings-block" });
  shortcutsBlock.createEl("h3", { text: "快捷指令（iOS）" });
  shortcutsBlock.createEl("p", {
    cls: "lifeos-settings-desc",
    text: "主屏推荐只放一个入口，打开 BrainCore 快速面板。在「快捷指令」App：新建 → 添加「打开 URL」→ 粘贴下方链接 → 添加到主屏幕。不必再套「打开 App → Obsidian」。",
  });
  const shortcutLinks = [
    { label: "快速面板（推荐）", url: "obsidian://braincore?action=quick" },
    { label: "捕捉", url: "obsidian://braincore?action=capture" },
    { label: "闪念胶囊", url: "obsidian://braincore?action=capsule" },
    { label: "最近文件", url: "obsidian://braincore?action=recent" },
    { label: "归档待办", url: "obsidian://braincore?action=archive" },
  ];
  ...
  shortcutLinks.forEach((item) => {
    shortcutsBlock.createEl("p", { cls: "lifeos-about-meta", text: item.label });
    const row = shortcutsBlock.createDiv({ cls: "lifeos-act-row" });
    const input = row.createEl("input", {
      type: "text",
      cls: "lifeos-act-input",
      attr: { readonly: "readonly", value: item.url, "aria-label": item.label },
    });
    input.onclick = () => input.select();
    const btn = row.createEl("button", { cls: "lifeos-act-btn", text: "复制", type: "button" });
    btn.onclick = () => void copyShortcutUrl(item.url);
  });
...
.bc-settings-compact .bc-habit-icon-input { width: 38px !important; min-width: 38px !important; text-align: center; font-size: 16px !important; border: none !important; background: transparent !important; }
...
function maybeShowLifeOsSuitePrompt(app, selfId, selfName) {
    const catalog = [
        ...
        { id: "braincore-dashboard", name: "BrainCore LifeOS" },
    ];
    const plugins = app.plugins?.plugins || {};
...
class BrainCoreSettingsTab extends PluginSettingTab {
    ...
    display() {
        ...
        if (!locked) tabDefs.push({ id: "paths", label: "路径" }, { id: "modules", label: "模块" }, { id: "habits", label: "习惯" });
        ...
        habitsCard.createEl('h3', { text: '📅 习惯打卡' });
...
class DashboardView extends ItemView {
    constructor(leaf, plugin) { 
        ...
        this.requestRefresh = customDebounce(() => {
    ...
    async onOpen() { 
        ...
        // 🚀 核心修复：扩大雷达监听范围！不仅监听 md 笔记，同时让白板、图片、表格等所有计入大盘的实体文件都能触发瞬间刷新
        const watchExts = ['md','canvas','png','jpg','jpeg','gif','webp','svg','xlsx','csv','pdf','doc','docx','ppt','pptx','txt','json','html','zip','rar','mp3','mp4','numbers','key'];
        ...
        const shouldRefreshForFile = (file) => {
            if (!file) return false;
            if (file.path === "Scripts/braincore-stats-history.json") return false;
            if (file.path.endsWith("/braincore-stats-history.json")) return false;
            if (file.path === "Scripts/braincore-stats-cache.json") return false;
            if (file.path.endsWith("/braincore-stats-cache.json")) return false;
            return watchExts.includes(file.extension);
        };
        const refreshForFile = (file) => {
            if (this.plugin._suppressDashboardRefresh) return;
            if (!shouldRefreshForFile(file)) return;
        ...
        this.refreshInterval = setInterval(() => {
            ...
            const fullEvery = this.app.isMobile ? 20 : 15;
    ...
    async onClose() { if (this.refreshInterval) clearInterval(this.refreshInterval); }
...
let workFile = allFiles.find(f =>
    ...
    weekFileReg.test(f.basename)
        ...
        if (!statsOnly && !tasksOnly && !workFile) workFile = this.plugin.findWeeklyWorkFile(bcMoment(now));
...
class BrainCoreIOSQuickModal extends Modal {
    ...
    injectQuickStyle() {
        new CaptureModal(this.app, this.plugin).injectStyles();
    ...
    async render() {
        ...
        await this.renderTasksBlock(wrap);
        this.renderHabitsBlock(wrap);

        // iOS 快捷面板不自动聚焦，避免勾选待办 / 打卡时弹出键盘。
    }
}
...
class BrainCorePlugin extends Plugin {
    ...
    openBrainCoreSettings(options = {}) {
        ...
        try {
            const pluginId = this.manifest?.id || "braincore-dashboard";
            this.app.setting.open();
            window.setTimeout(() => {
