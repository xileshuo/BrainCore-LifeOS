...
const PLUGIN_CHANGELOG = {
    "3.2.5": [
        "唤醒：从其他 App 切回时先让界面可点，再轻量补刷，全量重绘延后，减轻「假死」感",
        "打卡：监听 / 轮询插件 data.json；Mac 端打卡后控制台约 1～2 秒内同步勾选状态",
        "刷新：新增 habits 轻量作用域，外部改 habitData 时只重绘打卡区",
    ],
    "3.2.4": [
...
return this.app.vault.getFiles().find(f =>
    ...
    startPluginDataWatch() {
        ...
        // 桌面端约 1.5s；移动端稍慢省电
        const intervalMs = this.app.isMobile ? 4000 : 1500;
    ...
    async applyExternalPluginData(reason = "external") {
        ...
        try {
            ...
            const mergeKeys = [
                ...
                "pathIdeas", "pathTasks", "pathEssays", "pathWork", "pathClippings",
